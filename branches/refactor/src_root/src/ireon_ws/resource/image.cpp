 ...
 * $Id: image.cpp 314 2005-11-27 19:54:19Z zak $
...
void CImage::load(DataPtr data, const String& type)
{
	if( !data )
	{
		CLog::instance()->log(CLog::msgLvlError,_("Loading image from empty data stream.\n"));
		return;
	}
	...
	ilLoadL(ilType,static_cast< ILvoid* >(data->wdata()),static_cast< ILuint >(data->size()));

	// Check if everything was ok
	ILenum PossibleError = ilGetError() ;
	if( PossibleError != IL_NO_ERROR )
	{
		CLog::instance()->log(CLog::msgLvlError,_("Error loading image of type '%s'.\n"),type.c_str());
		return;
	}
	...
	{
		CLog::instance()->log(CLog::msgLvlError,_("Unsupported image format '%s'.\n"),type.c_str());
...
void CImage::load(const String& file)
{
	...
	if( pos == String::npos )
	{
		CLog::instance()->log(CLog::msgLvlError,_("Invalid file name, no extention: '%s'.\n"),file.c_str());
		return;
	}
