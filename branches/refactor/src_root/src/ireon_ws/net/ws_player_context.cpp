 ...
 * $Id: ws_player_context.cpp 433 2005-12-20 20:19:15Z zak $
...
{
	m_signals.push_back(&onLogin);
	m_signals.push_back(&onSendPos);
	m_signals.push_back(&onChatCmd);
	m_signals.push_back(&onFightCmd);
};
...
/*void CWSPlayerContext::sendFightPos(uint chId, uint enemyId, bool closeBattle)
{
	CData data;
	data.wrt((byte)(SIGNALS_START + 3));
	data.wrt((byte)FIGHT_ATTACK);
	data << chId << enemyId << closeBattle;
	write(data);
};*/
