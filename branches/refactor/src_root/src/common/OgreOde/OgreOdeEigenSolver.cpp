...
void EigenSolver::Tridiagonal3 ()
{
	...
	if ( fM02 != (Real)0.0 )
	{
		Real fLength = sqrtf(fM01*fM01+fM02*fM02);
...
void EigenSolver::GuaranteeRotation ()
{
	if ( !m_bIsRotation )
	{
		...
		for (int iRow = 0; iRow < m_iSize; iRow++) m_kMat[iRow][0] = -m_kMat[iRow][0];
	}
}
...
void EigenSolver::orthogonalLineFit(unsigned int vertex_count, const Vector3* vertices,Vector3& origin,Vector3& direction)
{
	...
	origin = vertices[0];
    for(i = 1; i < vertex_count; ++i) origin += vertices[i];
	Real fInvQuantity = 1.0/vertex_count;
	origin *= fInvQuantity;
	...
	{
		Vector3 kDiff = vertices[i] - origin;
...
Real EigenSolver::SqrDistance(const Vector3& rkPoint,const Vector3& origin,const Vector3& direction)
{
    ...
    Real fSqrLen = direction.squaredLength();
	Real fT = kDiff.dotProduct(direction)/fSqrLen;
    kDiff -= fT*direction;
...
Real& EigenSolver::operator() (int iRow, int iCol)
{
...
void EigenSolver::GetEigenvector (int i, Vector3& rkV) const
{
	...
	if ( m_iSize == 3 )
	{
		for (int iRow = 0; iRow < m_iSize; iRow++) rkV[iRow] = m_kMat[iRow][i];
	}
	else
	{
	    rkV = Vector3::ZERO;
...
void EigenSolver::GaussPointsFit(int iQuantity,const Vector3* akPoint,Vector3 &rkCenter,Vector3 akAxis[3],Real afExtent[3])
{
