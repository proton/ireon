...
Body* MeshInformer::createSingleDynamicSphere(SceneNode* node,Real mass,Space* space)
{
	MeshInformer mi;
	Body* body = 0;
	Real rad = 0.0;

	if(mi.addAllMeshes(node,Vector3::ZERO,Quaternion::IDENTITY,node->getScale()))
	{
		rad = mi.getRadius();
		if(rad > 0.0)
		{
			body = new Body("OgreOde::Body_" + node->getName());
			...
			body->setOrientation(node->getOrientation());
...
Body* MeshInformer::createSingleDynamicBox(SceneNode* node,Real mass,Space* space)
{
	MeshInformer mi;
	Body* body = 0;
...
BoxGeometry* MeshInformer::createSingleStaticBox(SceneNode* node,Space* space)
{
	MeshInformer mi;
	BoxGeometry* geom = 0;
	...
	if(mi.addAllMeshes(node,Vector3::ZERO,Quaternion::IDENTITY,node->getScale()))
	{
		geom = new BoxGeometry(mi.getSize(),space);

		geom->setPosition(node->getPosition());
		geom->setOrientation(node->getOrientation());
	}
