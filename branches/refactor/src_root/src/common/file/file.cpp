 ...
 * $Id: file.cpp 306 2005-11-26 21:30:52Z llyeli $
...
int CFile::write(const void* buffer, int numBytes)
{
	assert(this->isOpen());
...
#ifdef __WIN32__
	DWORD written;
	WriteFile(this->m_handle, buffer, numBytes, &written, NULL);
	return written;
#else
...
int CFile::read(void* buffer, int numBytes)
{
	assert(this->isOpen());
...
#ifdef __WIN32__
	DWORD read;
	ReadFile(this->m_handle, buffer, numBytes, &read, NULL);
	return read;
#else
