 ...
 * $Id: data.cpp 257 2005-11-24 22:41:17Z llyeli $
...
CData::CData(const char* data, uint length, bool allowWrite):
m_data(length)
{
	assert(data);
...
size_t CData::read(void *data, uint length)
{
	uint indent = 0;
...
size_t CData::sread(void *data, uint length, uint& indent)
{
	assert(data);
	assert(!m_store);
...
size_t CData::write(const void *data, uint length)
{
	uint indent = 0;
...
size_t CData::swrite(const void *data, uint length, uint& indent)
{
	assert(data);
	assert(m_store);
...
int CData::readStrLen(uint& indent)
{
 byte bLen;
