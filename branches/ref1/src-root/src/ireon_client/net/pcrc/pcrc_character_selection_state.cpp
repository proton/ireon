/**
 * @file ireon_client/net/pcrc/pcrc_character_selection_state.cpp
...
CPCRCCharacterSelectionState::CPCRCCharacterSelectionState(CPCRSConnection *ownerConnection):
m_ownerConnection(ownerConnection)
{
	setStateName("Character Selection State");
/*
	registerSlot(ireon::net::rspcCodes::scCharSelList, (CGenericSlot) &CRSCCharacterSelectionState::onListCharsRequest);
	registerSlot(ireon::net::rspcCodes::scCharSelSelect, (CGenericSlot) &CRSCCharacterSelectionState::onSelectCharRequest);
	registerSlot(ireon::net::rspcCodes::scCharSelCreate, (CGenericSlot) &CRSCCharacterSelectionState::onCreateCharRequest);
	registerSlot(ireon::net::rspcCodes::scCharSelRemove, (CGenericSlot) &CRSCCharacterSelectionState::onRemoveCharRequest);
*/
}

// ------------------------------------------------------------------------
/*
void CRSCCharacterSelectionState::onListCharsRequest (CData &data)
{
...
// ------------------------------------------------------------------------
/*	
void CRSCCharacterSelectionState::onSelectCharRequest (CData &data)
{
	int id;
	data >> id;
	...
	if(CRootDB::instance()->findCharacter(id) && CRootDB::instance()->findCharacter(id)->m_account == m_ownerConnection->getOwnerAccPtr()->getId())
	{
		...
		CRSCMConnection* con = CRootApp::instance()->getCMConnectionsManager()->findConnection(cluster);
		if( con )
		{
*/			/*
...
// ------------------------------------------------------------------------
/*
void CRSCCharacterSelectionState::onCreateCharRequest (CData& data)
{
...
// ------------------------------------------------------------------------
/*
void CRSCCharacterSelectionState::onRemoveCharRequest (CData& data)
{
	uint charId;
	data >> charId;
	...
	CData	request;
	request.wrt((ireon::net::commandId) ireon::net::rsccCodes::scCheckRemoveCharRequest);
	request << charId;
	con->processBlock(request);
