...
//    }
                                                                ...
                                                                const uint Lod) const
    {
        ...
        const int verdata_spread = 1 << fixedLod;
...
//        return y1 * (1-x_pct) + y2 * x_pct;
    ...
    const Vector3 PagingLandScapeData2DManager::getNormalAt( const uint dataX, const uint dataZ, 
                                                            const uint x, const uint z, bool upperTri) const
    {
	    PagingLandScapeData2D* data = mData2D[ dataX ][ dataZ ];
        assert (data);
	    if ( data->isLoaded() )
	    {
            ...
            #else /*_LOADEDNORM*/
            {
				...
				const Real Divider = 1.0f / PagingLandScapeOptions::getSingleton ().scale.x;
...
//                              (gH (x-1, z-1) + gH (x, z-1) + gH (x, z-1) + gH (x+1, z-1) - gH (x-1, z+1) - gH (x, z+1) - gH (x, z+1) - gH (x+1, z+1)) * Divider);
				if( upperTri )
				{
					Vector3 result((gH (x, z+1) - gH (x + 1, z+1)) * Divider,
									1.0f,
								(gH (x+1, z) - gH (x+1, z + 1 )) * Divider); 
				...
				{
					Vector3 result((gH (x, z) - gH (x + 1, z)) * Divider,
									1.0f,
								(gH (x, z) - gH (x, z + 1 )) * Divider); 
    ...
    //-----------------------------------------------------------------------
    const Vector3 PagingLandScapeData2DManager::getNormal(const Real x, const Real z) const
    {
        uint dataX, dataZ;
        if (mPageManager->getPageIndices(x, z, dataX, dataZ, false))
        {
            ...
            if ((mData2D[ dataX ][ dataZ ]->isLoaded()))
			{
				...
				localX -= dataX*pSize - mOptions->maxUnScaledX;
				localZ -= dataZ*pSize - mOptions->maxUnScaledZ;
                ...
                if( localX - static_cast<int>(localX) > 1 - (localZ - static_cast<int>(localZ)) )
	                return getNormalAt (dataX, dataZ, localX, localZ, true); /// Upper triangle
				else
	                return getNormalAt (dataX, dataZ, localX, localZ, false);/// Bottom triangle
			}
    ...
    //----------------------------------------------------------------------------
	void PagingLandScapeData2DManager::getTriInfo( Real x, Real z, uint& ix, uint& iz, bool& upper) const
	{
		uint pageX, pageZ;
		mPageManager->getPageIndices(x, z, pageX, pageZ, false);

		// scale position from world to page scale
		Real localX = x / mOptions->scale.x;
		Real localZ = z / mOptions->scale.z;
		ix = localX + mOptions->maxUnScaledX;
		iz = localZ + mOptions->maxUnScaledZ;
		// adjust x and z to be local to page
		const Real pSize = mOptions->PageSize - 1;
		...
		if (localZ < 0)
			...
			localZ = pSize - 1;
		Real deltaX = localX - static_cast<int>(localX);
		Real deltaZ = localZ - static_cast<int>(localZ);
		// z
		// ^
		// |--------|
		// |\       |
	...
	void PagingLandScapeData2DManager::getPageIndices(const Real& rx, const Real& rz, uint& x, uint& z, bool alwaysAnswer)
	{
		mPageManager->getPageIndices(rx,rz,x,z,alwaysAnswer);
	}
