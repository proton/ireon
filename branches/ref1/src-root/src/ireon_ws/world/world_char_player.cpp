 ...
 * $Id: world_char_player.cpp 610 2006-04-20 11:04:59Z zak $
...
void CWorldCharPlayer::update(uint time)
{
	...
	if( CWorld::m_processOutputPulse && m_context )
...
{
	CWorldCharacter::die(killer);
	if( m_context )
		m_context->sendCharactersPositions();
	CWorld::instance()->removeCharPlayer(getId());
};