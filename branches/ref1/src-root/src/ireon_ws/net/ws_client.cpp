 ...
 * $Id: ws_client.cpp 522 2006-03-06 16:49:30Z zak $
...
void CWSClient::onNewChar(CData& data)
{
	...
	d.serialize(data);
	data >> acc;
	CLog::instance()->log(CLog::msgLvlInfo,"Creating new character with id %d to account %d.\n",d.m_id,acc);
	CData answer;
	...
	if( res != CCE_NONE )
	{
		CLog::instance()->log(CLog::msgLvlInfo,"Character with id %d wasn't created.\n",d.m_id);
		answer << d.m_id;
	}
	else
	{
		CLog::instance()->log(CLog::msgLvlInfo,"Created character with id %d\n",d.m_id);
