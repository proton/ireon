...
void pingpong(CSockLayer* server,CSockLayer* client,SDLNet_SocketSet m_socketSetClient)
{
...
void testCSockLayerIO()
{
	BOOST_CHECKPOINT("Test CSockLayer Input-Output System");
	...
	{
	CSockLayer server(socketServer, m_socketSet);
	SDL_Delay(100);
...
test_suite* init_unit_test_suite( int /*argc*/, char* /*argv*/[] ) {
	...
	test_suite* test = BOOST_TEST_SUITE("CSockLayer test");
	test->add( BOOST_TEST_CASE( &testCCompressingLayerIO));
	test->add( BOOST_TEST_CASE( &testCSockLayerIO ));
