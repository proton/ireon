...
#include <math.h>
#include "exception"
class EFixedPointMathError: public std::exception {};
...
#undef FIX_DEBUG // skip debug

#ifdef FIX_DEBUG
	int m_dec;
	int m_float;
#endif
	inline void _debug_func()
	{
		#ifdef FIX_DEBUG
		m_dec =  m_nVal / 10000L;
		m_float = abs(long(m_nVal - m_dec * 10000L)); //RESOLUTION
		#endif
	};
...
public:
	...
	operator int(void);
	fixed floor(void);
	fixed floor( int resolution);
	fixed ceil(void);
	fixed add(fixed b);
	fixed add(fixed b) const;
	fixed subtract(fixed b);
	fixed multiply(const fixed b);
	fixed divide(const fixed b);
