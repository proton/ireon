...
inline Packet *AllocPacket(unsigned dataSize)
{
...
inline Packet *AllocPacket(unsigned dataSize, unsigned char *data)
{
...
{
	int sum, quantity;
	RemoteSystemStruct *remoteSystem = GetRemoteSystemFromPlayerID( playerId, false );
...
{
	RemoteSystemStruct * remoteSystem = GetRemoteSystemFromPlayerID( target, false );
...
RPCMap* RakPeer::GetRPCMap( const PlayerID playerId)
{
	...
	{
		RemoteSystemStruct *rss=GetRemoteSystemFromPlayerID(playerId, false);
		if (rss)
			return &(rss->rpcMap);
		else
			return 0;
...
RakNetStatisticsStruct * const RakPeer::GetStatistics( const PlayerID playerId )
{
	...
	rss = GetRemoteSystemFromPlayerID( playerId, false );
...
bool RakPeer::SendConnectionRequest( const char* host, unsigned short remotePort, char* passwordData, int passwordDataLength )
{
	...
	// Already connected?
	if (GetRemoteSystemFromPlayerID(playerId, false))
		return false;
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
RakPeer::RemoteSystemStruct *RakPeer::GetRemoteSystemFromPlayerID( const PlayerID playerID, bool calledFromNetworkThread ) const
{
	unsigned i;
...
void RakPeer::NotifyAndFlagForDisconnect( const PlayerID playerId, bool performImmediate, unsigned char orderingChannel )
{
	...
	if (performImmediate)
	{
		...
		RemoteSystemStruct *rss=GetRemoteSystemFromPlayerID(playerId, true);
...
RakNetTime RakPeer::GetBestClockDifferential( const PlayerID playerId ) const
{
	...
	RemoteSystemStruct *remoteSystem = GetRemoteSystemFromPlayerID( playerId, true );
...
//	unsigned int bitLength;
	...
	*/
	...
	if (nameIsEncoded && GetRemoteSystemFromPlayerID(playerId, false))
	{
...
//		char data[MAX_OFFLINE_DATA_LENGTH];
	...
	else if ((unsigned char)(data)[0] == ID_OPEN_CONNECTION_REQUEST && length == sizeof(unsigned char)*2)
	{
		...
		RakPeer::RemoteSystemStruct *rss = rakPeer->GetRemoteSystemFromPlayerID( playerId, true );
...
//	unsigned numberOfBytesUsed;
	...
	while ((bcs=bufferedCommands.ReadLock())!=0)
	{
		if (bcs->command==BufferedCommandStruct::BCS_SEND)
		{
			...
			{
				remoteSystem=GetRemoteSystemFromPlayerID( bcs->playerId, true );
			//	if (remoteSystem==0)
	...
	//	remoteSystemList[ remoteSystemIndex ].allowPlayerIdAssigment=true;
		if ( remoteSystemList[ remoteSystemIndex ].isActive )
		{
			...
			if ( remoteSystem->reliabilityLayer.IsDeadConnection() ||
				...
				((remoteSystem->connectMode!=RemoteSystemStruct::CONNECTED && timeMS > remoteSystem->connectionTime && timeMS - remoteSystem->connectionTime > 10000))
				)
			{
