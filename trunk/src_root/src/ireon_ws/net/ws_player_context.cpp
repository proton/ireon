 ...
 * $Id: ws_player_context.cpp 752 2006-09-23 19:12:44Z zak $
...
void CWSPlayerContext::onClose()
{
	...
	if( getCharacter() )
	{
		CWSWorld::instance()->removeEntity(getCharacter()->getId());
...
void CWSPlayerContext::enterGame(AccPtr a)
{
	...
	{
		CLog::instance()->log(CLog::msgLvlInfo,__FUNCTION__,_("Account %s logged in from %s.\n"),a->getName(),getHost());
		answer.wrt((byte)1);
		...
		m_account = a;
//  		m_character = CWSWorld::instance()->findCharPlayer(a->getSelChar());
// 		if( !m_character )
		if(true)
		{
			...
			if( !m_character )
			{
				...
				CLog::instance()->log(CLog::msgLvlCritical,__FUNCTION__,_("Enter world: can't find character with id %d.\n"),a->getSelChar());
				answer.wrt((byte)0);
				write(answer);
				return;
			};
			CWSWorld::instance()->insertEntity(m_character);
		};
	...
	{
		answer.wrt((byte)0);
		CLog::instance()->log(CLog::msgLvlInfo,__FUNCTION__,_("Login from %s failed.\n"),getHost());
	}
	write(answer);
};
...
{
	CLog::instance()->log(CLog::msgLvlInfo,__FUNCTION__,_("Login attempt from %s.... "),getHost());
	String login;
	String passwd;
	data >> login >> passwd;
	m_login = login;
...
*//*
				CharacterPtr ch = CWSWorld::instance()->findEntityById(enemy);
				if( !ch )
*/
//				CLog::instance()->log(CLog::msgLvlInfo,__FUNCTION__,"FIGHT: Can't find enemy with id %d for character %s(%d).\n",enemy,m_character->getName(),m_character->getId());
/*				else
...
void CWSPlayerContext::onResurrect(CData& data)
{
	if( m_character->inWorld() )
		return;
	CData answer;
// 	m_character = CWSWorld::instance()->findCharPlayer(m_account->getSelChar());
// 	if( !m_character )
	if(true)
	{
		///Character isn't in world
